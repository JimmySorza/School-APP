  ...
  <DetailsLayout>
    <template #details>
      ...
      <div class="flex items-center heading-1">
        <span v-if="!getPrivateMode">{{ getAdministrator.name }}</span>
        <span v-else>{{ getAdministrator.id_number }}</span>

        <BaseTag
          class="text-white capitalize ml-4"
          :class="getRiskLevelBackground(getAdministrator.risk_level)"
        >
          {{ getAdministrator.risk_level }} Risk
        </BaseTag>
      </div>
      ...
      <div v-if="!getPrivateMode">
        ...
        <div class="body-1 mb-8 mt-1">
          ID Number: {{ getAdministrator.id_number }}
        </div>

        <div class="flex items-center body-1 w-1/2">
          <div class="w-2/5">
            <i class="fas fa-globe mr-2 w-5 text-center text-grey-default" />
            Administrator
          </div>
          ...
          <div>
            ...
            <span v-if="getAdministrator.phone_number">
              {{ getAdministrator.phone_number }}
            </span>
        ...
        <div class="flex items-center body-1 w-1/2 mt-1">
          ...
          <div class="w-2/5">
            ...
            <span v-if="getAdministrator.email">
          ...
          <div>
            ...
            <span v-if="getAdministrator.address">
              {{ getAdministrator.address }}
            </span>
    ...
    <template #content>
      <div class="flex items-center justify-between mb-8">
        <BaseTabs
          v-model="selectedTab"
          class="w-1/2"
          :tabs="getAdministratorsDetailsTabs"
        />
...
export default {
  ...
  computed: {
    ...
    // Get Administrator
    getAdministrator () {
      if (!this.$route.params.administrator) return {}
      return this.$route.params.administrator
    },
    getAdministratorsDetailsTabs () {
      return [
        {
  ...
  mounted () {
    this.selectedTab = this.getAdministratorsDetailsTabs[0]
  }
}
</script>
